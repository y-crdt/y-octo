...
test.skip("testDeltaBug2", (t) => {
  ...
  const ytext = ydoc.getOrCreateText("id");
...
test.skip("testBasicFormat", (t) => {
  ...
  t.deepEqual(text0.toDelta(), [{ insert: "abc", attributes: { bold: true } }]);
  t.deepEqual(delta, [{ insert: "abc", attributes: { bold: true } }]);
  text0.delete(0, 1);
  t.assert(
    text0.toString() === "bc",
    "Basic delete on formatted works (position 0)",
  );
  t.deepEqual(text0.toDelta(), [{ insert: "bc", attributes: { bold: true } }]);
  t.deepEqual(delta, [{ delete: 1 }]);
  text0.delete(1, 1);
  t.assert(text0.toString() === "b", "Basic delete works (position 1)");
  t.deepEqual(text0.toDelta(), [{ insert: "b", attributes: { bold: true } }]);
  t.deepEqual(delta, [{ retain: 1 }, { delete: 1 }]);
  text0.insert(0, "z", { bold: true });
  t.assert(text0.toString() === "zb");
  t.deepEqual(text0.toDelta(), [{ insert: "zb", attributes: { bold: true } }]);
  t.deepEqual(delta, [{ insert: "z", attributes: { bold: true } }]);
  // @ts-ignore
  t.assert(
    text0._start.right.right.right.content.str === "b",
    "Does not insert duplicate attribute marker",
  );
  text0.insert(0, "y");
  t.assert(text0.toString() === "yzb");
  t.deepEqual(text0.toDelta(), [
    { insert: "y" },
    { insert: "zb", attributes: { bold: true } },
  ]);
  t.deepEqual(delta, [{ insert: "y" }]);
...
test.skip("testFalsyFormats", (t) => {
  ...
  t.deepEqual(delta, [{ insert: "abcde", attributes: { falsy: false } }]);
  text0.format(1, 3, { falsy: true });
  t.deepEqual(text0.toDelta(), [
    { insert: "a", attributes: { falsy: false } },
    { insert: "bcd", attributes: { falsy: true } },
    { insert: "e", attributes: { falsy: false } },
  ]);
  t.deepEqual(delta, [
    { retain: 1 },
    { retain: 3, attributes: { falsy: true } },
  ]);
  text0.format(2, 1, { falsy: false });
  t.deepEqual(text0.toDelta(), [
    { insert: "a", attributes: { falsy: false } },
    { insert: "b", attributes: { falsy: true } },
    { insert: "c", attributes: { falsy: false } },
    { insert: "d", attributes: { falsy: true } },
    { insert: "e", attributes: { falsy: false } },
  ]);
  t.deepEqual(delta, [
    { retain: 2 },
...
test.skip("testMultilineFormat", (t) => {
  const ydoc = new Y.Doc();
  const testText = ydoc.getOrCreateText("test");
...
test.skip("testNotMergeEmptyLinesFormat", (t) => {
  const ydoc = new Y.Doc();
  const testText = ydoc.getOrCreateText("test");
  testText.applyDelta([
    { insert: "Text" },
...
test.skip("testPreserveAttributesThroughDelete", (t) => {
  const ydoc = new Y.Doc();
  const testText = ydoc.getOrCreateText("test");
  testText.applyDelta([
    { insert: "Text" },
...
test.skip("testTypesAsEmbed", (t) => {
  ...
  t.deepEqual(text0.toDelta()[0].insert.toJSON(), { key: "val" });
  let firedEvent = false;
  text1.observe((event) => {
    const d = event.delta;
    t.assert(d.length === 1);
    t.deepEqual(
      d.map((x) => /** @type {Y.AbstractType<any>} */ x.insert.toJSON()),
      [{ key: "val" }],
    );
    firedEvent = true;
  });
  ...
  t.deepEqual(delta[0].insert.toJSON(), { key: "val" });
...
test.skip("testSnapshot", (t) => {
  ...
  t.deepEqual(state1, [{ insert: "abcd" }]);
  const state2 = text0.toDelta(snapshot2);
  t.deepEqual(state2, [{ insert: "axcd" }]);
...
test.skip("testSnapshotDeleteAfter", (t) => {
  const { text0 } = init(gen, { users: 1 });
  const doc0 = text0.doc;
  doc0.gc = false;
  text0.applyDelta([
...
test.skip("testFormattingRemovedInMidText", (t) => {
  const { text0 } = init(gen, { users: 1 });
  text0.insert(0, "1234");
 ...
 */
test.skip("testFormattingDeltaUnnecessaryAttributeChange", (t) => {
  const { text0, text1, testConnector } = init(gen, { users: 2 });
  text0.insert(0, "\n", {
  ...
  t.deepEqual(filteredDeltas[0], [
    { retain: 1, attributes: { LIST_STYLES: "number" } },
  ]);
  t.deepEqual(filteredDeltas[0], filteredDeltas[1]);
});
...
test.skip("testLargeFragmentedDocument", (t) => {
  const itemsToInsert = largeDocumentSize;
  let update = /** @type {any} */ null;
  (() => {
    const doc1 = new Y.Doc();
    const text0 = doc1.getOrCreateText("txt");
    tryGc();
...
test.skip("testIncrementalUpdatesPerformanceOnLargeFragmentedDocument", (t) => {
  const itemsToInsert = largeDocumentSize;
  ...
  (() => {
    t.measureTime(
      `time to merge ${itemsToInsert} updates (differential updates)`,
      () => {
    ...
    tryGc();
    t.measureTime(
      `time to merge ${itemsToInsert} updates (ydoc updates)`,
      () => {
        const ydoc = new Y.Doc();
...
test.skip("testFormattingBug", (t) => {
  ...
  t.deepEqual(text1.toDelta(), expectedResult);
  t.deepEqual(text1.toDelta(), text2.toDelta());
 ...
 */
test.skip("testDeleteFormatting", (t) => {
  const doc = new Y.Doc();
  ...
  t.deepEqual(text.toDelta(), expected);
  t.deepEqual(text2.toDelta(), expected);
});
...
const marksChoices = [undefined, ...marks];
 ...
 * @type Array<function(any,prng.PRNG):void>
 */
const qChanges: Array<(arg0: any, arg1: prng.PRNG) => void> = [
  /**
   * @param {Y.Doc} y
   * @param {prng.PRNG} gen
   */
  ...
  (y: Y.Doc, gen: prng.PRNG) => {
     ...
     * @type {Array<any>}
     */
    const ops: Array<any> = currentPos > 0 ? [{ retain: currentPos }] : [];
    // create max 3 ops
      ...
      prng.oneOf(gen, [
        () => {
          ...
          const retain = math.min(
            prng.int32(gen, 0, contentLen - currentPos),
            5,
          );
          const format = prng.oneOf(gen, marks);
...
const checkResult = (result: any) => {
  for (let i = 1; i < result.testObjects.length; i++) {
     ...
     * @param {any} d
     */
    const typeToObject = (d: any) =>
      Y.isAbstractType(d.insert) ? d.insert.toJSON() : d;
