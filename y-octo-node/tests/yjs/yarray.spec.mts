import assert, { equal, deepEqual, doesNotThrow } from "node:assert";
...
test("yjs array test", { concurrency: false }, async (t) => {
  await t.test("testBasicUpdate", () => {
    const doc1 = new Y.Doc();
    const doc2 = new Y.Doc();
    doc1.getOrCreateArray("array").insert(0, ["hi"]);
    const update = Y.encodeStateAsUpdate(doc1);
    Y.applyUpdate(doc2, update);
    deepEqual(doc2.getOrCreateArray("array").toArray(), ["hi"]);
  });

  await t.test("testSlice", () => {
    const doc1 = new Y.Doc();
    const arr = doc1.getOrCreateArray("array");
    arr.insert(0, [1, 2, 3]);
    deepEqual(arr.slice(0), [1, 2, 3]);
    deepEqual(arr.slice(1), [2, 3]);
    deepEqual(arr.slice(0, -1), [1, 2]);
    arr.insert(0, [0]);
    deepEqual(arr.slice(0), [0, 1, 2, 3]);
    deepEqual(arr.slice(0, 2), [0, 1]);
  });
  ...
  const testArrayFrom = () => {
    ...
    deepEqual(nestedArray1.toArray(), [0, 1, 2]);
  };

  /**
   * Debugging yjs#297 - a critical bug connected to the search-marker approach
   */
  await t.test("testLengthIssue", () => {
    const doc1 = new Y.Doc();
    const arr = doc1.getOrCreateArray("array");
    arr.push([0, 1, 2, 3]);
    arr.delete(0);
    arr.insert(0, [0]);
    equal(arr.length, arr.toArray().length);
    doc1.transact(() => {
      arr.delete(1);
      equal(arr.length, arr.toArray().length);
      arr.insert(1, [1]);
  ...
  const testInsertThreeElementsTryRegetProperty = (tc: t.TestCase) => {
    const { testConnector, users, array0, array1 } = init(tc, { users: 2 });
    assert(array0);
    assert(array1);
    array0.insert(0, [1, true, false]);
    deepEqual(array0.toJSON(), [1, true, false], ".toJSON() works");
    testConnector.flushAllMessages();
    deepEqual(array1.toJSON(), [1, true, false], ".toJSON() works after sync");
    compare(users);
  };

  const testConcurrentInsertWithThreeConflicts = (tc: t.TestCase) => {
  ...
  const testConcurrentInsertDeleteWithThreeConflicts = (tc: t.TestCase) => {
  ...
  const testInsertionsInLateSync = (tc: t.TestCase) => {
    const { testConnector, users, array0, array1, array2 } = init(tc, {
      users: 3,
    });
    assert(array0);
  ...
  const testDisconnectReallyPreventsSendingMessages = (tc: t.TestCase) => {
    ...
    array0.insert(1, ["user0"]);
    array1.insert(1, ["user1"]);
    deepEqual(array0.toJSON(), ["x", "user0", "y"]);
    deepEqual(array1.toJSON(), ["x", "user1", "y"]);
    users[1].connect();
    users[2].connect();
    compare(users);
  };

  const testDeletionsInLateSync = (tc: t.TestCase) => {
    const { testConnector, users, array0, array1 } = init(tc, { users: 2 });
    assert(array0);
    assert(array1);
    array0.insert(0, ["x", "y"]);
  ...
  const testInsertThenMergeDeleteOnSync = (tc: t.TestCase) => {
    const { testConnector, users, array0, array1 } = init(tc, { users: 2 });
    assert(array0);
    assert(array1);
    array0.insert(0, ["x", "y", "z"]);
  ...
  const testInsertAndDeleteEvents = (tc: t.TestCase) => {
  ...
  const testNestedObserverEvents = (tc: t.TestCase) => {
    const { array0, users } = init(tc, { users: 2 });
    assert(array0);
    const vals: number[] = [];
    array0.observe((e) => {
    ...
    array0.insert(0, [0]);
    deepEqual(vals, [0, 1]);
    deepEqual(array0.toArray(), [0, 1]);
    compare(users);
  };

  const testInsertAndDeleteEventsForTypes = (tc: t.TestCase) => {
    const { array0, users } = init(tc, { users: 2 });
    assert(array0);
    let event: Record<string, any> | null = null;
    array0.observe((e) => {
   ...
   */
  const testObserveDeepEventOrder = (tc: t.TestCase) => {
    const { array0, users } = init(tc, { users: 2 });
    assert(array0);
    let events: any[] = [];
  ...
  const testObservedeepIndexes = () => {
    ...
    deepEqual(eventPath, ["my-array", 3]);
  };

  const testChangeEvent = (tc: t.TestCase) => {
    const { array0, users } = init(tc, { users: 2 });
    assert(array0);
    ...
    assert(
      changes !== null &&
        changes.added.size === 2 &&
        changes.deleted.size === 0,
    );
    deepEqual(changes.delta, [{ insert: [newArr, 4, "dtrn"] }]);
    changes = null;
    array0.delete(0, 2);
    assert(
      changes !== null &&
        changes.added.size === 0 &&
        changes.deleted.size === 2,
    );
    deepEqual(changes.delta, [{ delete: 2 }]);
    changes = null;
    array0.insert(1, [0.1]);
    assert(
      changes !== null &&
        changes.added.size === 1 &&
        changes.deleted.size === 0,
    );
    deepEqual(changes.delta, [{ retain: 1 }, { insert: [0.1] }]);
    compare(users);
  };

  const testInsertAndDeleteEventsForTypes2 = (tc: t.TestCase) => {
    const { array0, users } = init(tc, { users: 2 });
    assert(array0);
    ...
    array0.delete(1);
    equal(events.length, 2, "Event is triggered exactly once for deletion");
    compare(users);
  };
   ...
   * @param {t.TestCase} tc
   */
  const testNewChildDoesNotEmitEventInTransaction = (tc: t.TestCase) => {
    const { array0, users } = init(tc, { users: 2 });
    assert(array0);
    let fired = false;
    users[0].transact(() => {
  ...
  const testGarbageCollector = (tc: t.TestCase) => {
   ...
   * @param {t.TestCase} tc
   */
  const testEventTargetIsSetCorrectlyOnLocal = (tc: t.TestCase) => {
  ...
  const testEventTargetIsSetCorrectlyOnRemote = (tc: t.TestCase) => {
  ...
  const testIteratingArrayContainingTypes = () => {
    const y = new Y.Doc();
   ...
   * @param {t.TestCase} tc
   */
  const testRepeatGeneratingYarrayTests6 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 6);
  };

  const testRepeatGeneratingYarrayTests40 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 40);
  };

  const testRepeatGeneratingYarrayTests42 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 42);
  };

  const testRepeatGeneratingYarrayTests43 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 43);
  };

  const testRepeatGeneratingYarrayTests44 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 44);
  };

  const testRepeatGeneratingYarrayTests45 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 45);
  };

  const testRepeatGeneratingYarrayTests46 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 46);
  };

  const testRepeatGeneratingYarrayTests300 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 300);
  };

  const testRepeatGeneratingYarrayTests400 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 400);
  };

  const testRepeatGeneratingYarrayTests500 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 500);
  };

  const testRepeatGeneratingYarrayTests600 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 600);
  };

  const testRepeatGeneratingYarrayTests1000 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 1000);
  };

  const testRepeatGeneratingYarrayTests1800 = (tc: t.TestCase) => {
    applyRandomTests(tc, arrayTransactions, 1800);
  };

  const testRepeatGeneratingYarrayTests3000 = (tc: t.TestCase) => {
    t.skip(!t.production);
    applyRandomTests(tc, arrayTransactions, 3000);
  };

  const testRepeatGeneratingYarrayTests5000 = (tc: t.TestCase) => {
    t.skip(!t.production);
    applyRandomTests(tc, arrayTransactions, 5000);
  };

  const testRepeatGeneratingYarrayTests30000 = (tc: t.TestCase) => {
    t.skip(!t.production);
