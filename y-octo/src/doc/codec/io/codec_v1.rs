...
impl RawDecoder {
    pub fn new(buffer: Vec<u8>, interner: Arc<ThreadedRodeo>) -> Self {
        Self {
...
mod tests {
    ...
    fn test_crdt_reader() {
        {
            let mut reader = RawDecoder::new(vec![0xf2, 0x5], Default::default());
            assert_eq!(reader.read_var_u64().unwrap(), 754);
        }
        {
            let buffer = vec![0x5, b'h', b'e', b'l', b'l', b'o'];
            let mut reader = RawDecoder::new(buffer.clone(), Default::default());
        ...
        {
            let mut reader = RawDecoder::new(vec![0x40, 0x49, 0x0f, 0xdb], Default::default());
            assert_eq!(reader.read_f32_be().unwrap(), 3.1415927);
        }
        {
            let mut reader = RawDecoder::new(vec![0x40, 0x09, 0x21, 0xfb, 0x54, 0x44, 0x2d, 0x18], Default::default());
            assert_eq!(reader.read_f64_be().unwrap(), 3.141592653589793);
        }
        {
            let mut reader = RawDecoder::new(vec![0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff], Default::default());
            assert_eq!(reader.read_i64_be().unwrap(), i64::MAX);
        }
        {
            let mut reader = RawDecoder::new(vec![0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], Default::default());
            assert_eq!(reader.read_i64_be().unwrap(), i64::MIN);
        }
        {
            let mut reader = RawDecoder::new(vec![0x80], Default::default());
            assert_eq!(reader.read_info().unwrap(), 0x80);
        }
        {
            let mut reader = RawDecoder::new(vec![0x1, 0x2], Default::default());
